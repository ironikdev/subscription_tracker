    ...
    if(!subscription || subscription.status !== "active"){
        ...
        for(const dayBefore of REMINDERS){
            ...
            if(reminderDate.isAfter(dayjs())){
                await sleepUntilReminder(context,label:`Reminder ${daysBefore} days before`, remainderDate)
...
const sleepUntilReminder=async(context,label,date  ) =>{console.log(`Sleeping until ${label} reminder at ${date}`);
await context.sleepUntil(label,date.toDate());
}
const triggerReminder=async(context,label)=>{
    return await context.run(label,()=>{
        console.log(`Triggering reminder at ${label}`);
        //send email sms any push notification
